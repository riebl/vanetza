...
CertificateValidity DefaultCertificateValidatorV3::check_certificate(const CertificateV3& certificate)
{
    // if (!extract_validity_time(certificate)) {
    //     return CertificateInvalidReason::Broken_Time_Period;
    // }
    ...
    for (auto& possible_signer : m_cert_cache.lookup(signer_hash)) {
        auto verification_key = possible_signer.get_public_key(m_crypto_backend);
        if (!verification_key) {
            continue;
        }
