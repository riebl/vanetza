...
CertificateCacheV3::CertificateCacheV3(const Runtime& rt) : m_runtime(rt)
{
}

void CertificateCacheV3::insert(const CertificateV3& certificate)
{
    const HashedId8 id = certificate.calculate_hash();

    // this may drop expired entries and extend some's lifetime
    std::list<CertificateV3> certs = lookup(id);

    // TODO: implement equality comparison for Certificate
    if (certs.size()) {
        const auto binary_insert = certificate.convert_for_signing();
        for (auto& cert : certs) {
            const auto binary_found = certificate.convert_for_signing();
            if (binary_insert == binary_found) {
                return;
            }
        }
...
std::list<CertificateV3> CertificateCacheV3::lookup(const HashedId8& id)
{
    drop_expired();

    using iterator = std::multimap<HashedId8, CachedCertificateV3>::iterator;
    std::pair<iterator, iterator> range = m_certificates.equal_range(id);

    std::list<CertificateV3> matches;
    for (auto item = range.first; item != range.second; ++item) {
        const CertificateV3& cert = item->second.certificate;
...
void CertificateCacheV3::drop_expired()
{
    while (!m_expiries.empty() && is_expired(m_expiries.top())) {
        m_certificates.erase(m_expiries.top().certificate);
        m_expiries.pop();
...
bool CertificateCacheV3::is_expired(const Expiry& expiry) const
{
    return m_runtime.now() > expiry;
}

void CertificateCacheV3::refresh(heap_type::handle_type& handle, Clock::duration lifetime)
{
    static_cast<Clock::time_point&>(*handle) = m_runtime.now() + lifetime;
    m_expiries.update(handle);
}

CertificateCacheV3::Expiry::Expiry(Clock::time_point expiry, map_type::iterator it) :
    Clock::time_point(expiry), certificate(it)
{
}
