...
bool check_certificate_time(CertificateV3& certificate, Clock::time_point now)
{
...
bool check_certificate_region(CertificateV3& certificate, const PositionFix& position)
{
...
bool assign_permissions(CertificateV3& certificate, VerifyConfirm& confirm)
{
...
{
    return [&](VerifyRequestV3&& request) -> VerifyConfirm {
        // TODO check if certificates in chain have been revoked for all CA certificates, ATs are never revoked
        ...
        if (requested_certs.size() > 0) {
            for (auto& requested_cert : requested_certs) {
                if (truncate(cert_provider.own_certificate().calculate_hash()) == requested_cert) {
                    sign_policy.request_certificate();
                }

                for (auto& cert : cert_provider.own_chain()) {
                    if (truncate(cert.calculate_hash()) == requested_cert) {
                        sign_policy.request_certificate_chain();
                    }
                }
            }
        ...
        const std::unique_ptr<SignerInfoV3> signer_info(new SignerInfoV3(secured_message.get_signer_info()));
        std::list<CertificateV3> possible_certificates;
        bool possible_certificates_from_cache = false;
        ...
        if (signer_info) {
            switch (get_type(*signer_info)) {
                case SignerInfoType::Certificate:
                    possible_certificates.push_back(boost::get<CertificateV3>(*signer_info));
                    signer_hash = (boost::get<CertificateV3>(*signer_info)).calculate_hash();
                ...
                case SignerInfoType::Certificate_Chain:
                {
                    std::list<CertificateV3> chain = boost::get<std::list<CertificateV3>>(*signer_info);
                    if (chain.size() == 0) {
                        confirm.report = VerificationReport::Signer_Certificate_Not_Found;
                        return confirm;
                    } else if (chain.size() > 3) {
                    ...
                    if (chain.size()>1){
                        ...
                        std::list<CertificateV3>::iterator it = chain.begin();
                        std::advance(it, 1);
                        CertificateV3& cert = *it;
                        CertificateValidity validity = certs.check_certificate(cert);
                    ...
                    signer_hash = chain.front().calculate_hash();
        ...
        boost::optional<CertificateV3> signer;
        ...
        for (const auto& cert : possible_certificates) {
            ...
            boost::optional<ecdsa256::PublicKey> public_key = cert.get_public_key(backend);
...
VerifyServiceV3 dummy_verify_serviceV3(VerificationReport report, CertificateValidity validity)
{
    return [=](VerifyRequestV3&& request) -> VerifyConfirm {
        ...
        confirm.report = report;
        confirm.certificate_validity = validity;
        confirm.its_aid = vanetza::ItsAid(request.secured_message.get_psid());
        return confirm;
    };
}
