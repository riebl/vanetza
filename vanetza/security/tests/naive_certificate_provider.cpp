...
{
    CertificateVariant signed_certificate = cert_provider.own_certificate();

    struct own_certificate_visitor : public boost::static_visitor<void>
        {
            void operator()(const vanetza::security::Certificate& signed_certificate) const
            {
                // Check signature
        ...
        boost::apply_visitor(own_certificate_visitor(), signed_certificate);
}
