...
DelegatingSecurityEntityV3::DelegatingSecurityEntityV3(SignServiceV3 sign, VerifyServiceV3 verify) :
    m_sign_service(std::move(sign)),
    m_verify_service(std::move(verify))
{
    if (!m_sign_service) {
...
EncapConfirmV3 DelegatingSecurityEntityV3::encapsulate_packet(EncapRequest&& encap_request)
{
    SignRequest sign_request;
    sign_request.plain_message = std::move(encap_request.plaintext_payload);
    sign_request.its_aid = encap_request.its_aid;

    SignConfirmV3 sign_confirm = m_sign_service(std::move(sign_request));
    EncapConfirmV3 encap_confirm;
    encap_confirm.sec_packet = std::move(sign_confirm.secured_message);
    return encap_confirm;
}

DecapConfirm DelegatingSecurityEntityV3::decapsulate_packet(DecapRequestV3&& decap_request)
{
    VerifyConfirm verify_confirm = m_verify_service(VerifyRequestV3 { decap_request.sec_packet });
    DecapConfirm decap_confirm;
    decap_confirm.plaintext_payload = std::move(CohesivePacket(decap_request.sec_packet.get_payload(), OsiLayer::Session));
    decap_confirm.report = static_cast<DecapReport>(verify_confirm.report);
    decap_confirm.certificate_validity = verify_confirm.certificate_validity;
    decap_confirm.its_aid = verify_confirm.its_aid;
    decap_confirm.permissions = verify_confirm.permissions;
