...
public:
	...
	void request(const dcc::DataRequest& request,
			std::unique_ptr<ChunkPacket> packet) {
		buffers_[0] = create_ethernet_header(request.destination,
				request.source, request.ether_type);
		for (auto& layer : osi_layer_range<OsiLayer::Network,
		...
		//std::cout << "packet sent with " << bytes_sent << " bytes length\n";
	}

private:
	static constexpr std::size_t layers_ = num_osi_layers(OsiLayer::Link,
			OsiLayer::Application);
...
RouterContext::RouterContext(raw_protocol::socket& socket,
		const EthernetDevice& device, TimeTrigger& trigger) :
		mib_(configure_mib(device)), router_(trigger.runtime(), mib_), socket_(
				socket), device_(device), trigger_(trigger), request_interface_(
				new DccPassthrough(socket, trigger)), receive_buffer_(2048,
				0x00), receive_endpoint_(socket_.local_endpoint()) {
	router_.packet_dropped = std::bind(&RouterContext::log_packet_drop, this,
			std::placeholders::_1);
...
void RouterContext::do_receive() {
	...
	socket_.async_receive_from(asio::buffer(receive_buffer_), receive_endpoint_,
			std::bind(&RouterContext::on_read, this, sph::_1, sph::_2));
}

void RouterContext::on_read(const boost::system::error_code& ec,
		std::size_t read_bytes) {
  
    
	if (!ec) {
	  
	  std::cout << "after checking ec" << std::endl;
	  
		ByteBuffer buffer(receive_buffer_.begin(),
		...
		for (const LocationTable::entry_type& entry : router_.get_location_table().neighbours()) {
			...
			std::cout << "Printing Location Table" << std::endl;
...
void RouterContext::pass_up(CohesivePacket&& packet) {
	if (packet.size(OsiLayer::Link) < EthernetHeader::length_bytes) {
		std::cerr << "drop invalid packet (too short)\n";
	} else {
		packet.set_boundary(OsiLayer::Link, EthernetHeader::length_bytes);
		auto link_range = packet[OsiLayer::Link];
		EthernetHeader hdr = decode_ethernet_header(link_range.begin(),
				link_range.end());
		if (hdr.source != mib_.itsGnLocalGnAddr.mid()) {
			std::cout << "received " << packet.size() << " bytes from "
					<< hdr.source << "\n";
			std::unique_ptr < PacketVariant
					> up { new PacketVariant(std::move(packet)) };
