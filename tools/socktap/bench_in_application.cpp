...
BenchInApplication::BenchInApplication(const Runtime& rt, boost::asio::steady_timer& timer, milliseconds interval)
    : m_runtime(rt), m_interval(interval), m_timer(timer)
{
    schedule_timer();
}
