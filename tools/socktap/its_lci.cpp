...
int ITC_LCI_Application::createSocket(){
...
int ITC_LCI_Application::closeSocket(){
    return close(this->sockfd);
}

void ITC_LCI_Application::setSendToServer(bool send_to_server){
    this->send_to_server = send_to_server;
}

void ITC_LCI_Application::setServerPort(int serverPort){
    this->server_port = serverPort;
}

void ITC_LCI_Application::setServerIP(const char * serverIP){
...
void ITC_LCI_Application::setStationID(int station_id){
    this->station_id = station_id;
}

int  ITC_LCI_Application::sendToServer(u_int64_t* dataToSend, int size){
...
void ITC_LCI_Application::set_interval(Clock::duration interval)
{
...
void ITC_LCI_Application::print_generated_message(bool flag)
{
    print_tx_msg_ = flag;
}

void ITC_LCI_Application::print_received_message(bool flag)
{
    print_rx_msg_ = flag;
}

ITC_LCI_Application::PortType ITC_LCI_Application::port()
{
    return btp::ports::CAM;
}

int ITC_LCI_Application::decodeCAM(const asn1::Cam& recvd, char* message){
...
void ITC_LCI_Application::indicate(const DataIndication& indication, UpPacketPtr packet)
{
    ...
    if(cam && send_to_server){
        ...
        char message [500];
        int size = this->decodeCAM(*cam, message);
        this->sendToServer((u_int64_t*)message, size);
    }
    
}

void ITC_LCI_Application::schedule_timer()
{
    
    runtime_.schedule(cam_interval_, std::bind(&ITC_LCI_Application::on_timer, this, std::placeholders::_1), this);
}

void ITC_LCI_Application::on_timer(Clock::time_point)
{
    schedule_timer();
